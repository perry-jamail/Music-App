...
namespace DatabaseSQLMusicApp
{
    ...
    {
        string connectionString = "datasource=localhost;port=8889;username=root;password=root;database=music2;";
        ...
        public List<JObject> getTracksUsingJoin(int albumID)
        {
            ...
            MySqlCommand command = new MySqlCommand();
            command.CommandText = "SELECT tracks.ID AS trackID, albums.ALBUM_TITLE, `track_title`, `video_url`, `lyrics` FROM `tracks` JOIN albums ON albums_ID = albums.ID WHERE albums.ID = @albumid"; ;
            command.Parameters.AddWithValue("@albumid", albumID);
            command.Connection = connection;
